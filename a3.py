...
def title_by_year_range(matches: List[str]) -> List[str]:
    ...
    res = []
    for movie in movie_db:
        if get_year(movie) >= startYear and get_year(movie) <= endYear:
            res.append(get_title(movie))
    return res 
...
def title_by_director(matches: List[str]) -> List[str]:
    ...
    """
    movieDirector = matches[0]
    res = []
    for movie in movie_db:
        if get_director(movie) == movieDirector:
            res.append(get_title(movie))
    return res
